...
describe('Testing server routes', () => {
    it('Ruta para (/) sea codigo 200', () => {
        ...
        respuesta.end((error, data) => {
            chai.assert.equal(data.status, 200, 'Codigo de status no coincide con 200');
        });
    });
    ...
    it('Testing rutas not found with GET method.', () => {
        ...
        respuesta.end((error, data) => {
            chai.assert.equal(data.text, `Ruta GET no encontrada.`, 'No cumple con ruta not found');
        });
    });
    it('Testing rutas not found with POST method.', () => {
        let respuesta = chai.request(server).post('/testRoute');
        respuesta.end((error, data) => {
            chai.assert.equal(data.text, `Ruta POST no encontrada.`, 'No cumple con ruta not found');
        });
    });
    server.close();
    it('Testing rutas not found with PUT method.', () => {
        ...
        respuesta.end((error, data) => {
            chai.assert.equal(data.text, `Ruta PUT no encontrada.`, 'No cumple con ruta not found');
        });
    });
    server.close();
    it('Testing rutas not found with DELETE method.', () => {
        ...
        respuesta.end((error, data) => {
            chai.assert.equal(data.text, `Ruta DELETE no encontrada.`, 'No cumple con ruta not found');
        });
    });
    server.close();
});